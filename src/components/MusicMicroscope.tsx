...
function createJumpingCues(cues: ExplorationCue[]): ExplorationCue[] {
  return cues.map((cue, index) => ({
    ...cue,
    note: cue.note.replace(/(\d)$/, (_, octave: string) =>
      String(Math.max(1, Number(octave) + (index % 2 === 0 ? 1 : -1)))
    ),
...
export default function MusicMicroscope({
  cues,
  evidenceLabels,
  onNote,
  onReturn,
}: MusicMicroscopeProps) {
      ...
      try {
        ...
        for (const cue of sequence) {
          ...
          await new Promise<void>((resolve) =>
            window.setTimeout(resolve, getCueDurationMs(cue, 72))
          )
        }
  ...
  return (
    <section className="music-microscope" aria-labelledby="music-microscope-title">
      <header className="music-microscope__header">
        ...
        <button type="button" className="music-microscope__return" onClick={() => onReturn()}>
          回到作品再听
        </button>
      </header>
      ...
      <div className="music-microscope__body">
        ...
        <main className="music-microscope__preview">
          ...
          <div className="music-microscope__comparison">
            {(['flowing', 'jumping'] as PreviewVariant[]).map((variant) => (
              <article
                className={`music-microscope__comparison-card ${previewVariant === variant ? 'active' : ''}`}
                key={variant}
              >
                <div>
                  <span className="music-microscope__variant">
                    {variant === 'flowing' ? 'A' : 'B'}
                  </span>
                ...
                <button
                  type="button"
                  onClick={() =>
                    isPlaying && previewVariant === variant
                      ? stopPlayback()
                      : void playPreview(variant)
                  }
                >
          ...
          <div className="music-microscope__wave" aria-label="片段变化标记">
            {activeCues.map((cue, index) => (
              <span
                key={`${cue.note}-${index}`}
                className={index === markedCueIndex ? 'marked' : ''}
                style={{ height: `${28 + cue.velocity * 48}%` }}
              />
            ))}
          </div>
        ...
        <aside className="music-microscope__observation">
          ...
          <div className="music-microscope__evidence-list">
            {evidenceLabels.map((label) => (
              <button
                type="button"
                key={label}
                aria-pressed={selectedEvidence.includes(label)}
                className={selectedEvidence.includes(label) ? 'selected' : ''}
                onClick={() => toggleEvidence(label)}
              >
                {label}
              </button>
            ))}
          ...
          <button
            type="button"
            className="music-microscope__save"
            disabled={!canSave}
          ...
          {saveNotice && (
            <p className="music-microscope__notice" aria-live="polite">
              {saveNotice}
            </p>
          )}
        </aside>
